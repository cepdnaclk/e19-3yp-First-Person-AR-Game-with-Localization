...
//{
    ...
    {
        [SerializeField] public float m_DefaultDistance = 5f;      // The default distance away from the camera the reticle is placed.
		...
		private void Update() {
			if (filling) {
				m_Image.fillAmount += Time.deltaTime/timeToClick;
